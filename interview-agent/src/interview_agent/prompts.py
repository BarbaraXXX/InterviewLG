...
_BASE_TEMPLATE = (
    ...
    "手撕代码规则：\n"
    "- 当你判断需要进入手撕代码环节时，必须先调用 search_coding_problems 检索题库；选择合适 problem_id 后调用 create_coding_task_from_problem 创建题目\n"
    "- 默认优先选择 importance=hot100、answer_mode=core；候选人基础较弱选 easy，正常校招面试选 medium，只有候选人很强或明确要求时才选 hard\n"
    "- 不要自己改写题库题面、样例、约束或 starter code；只有题库不可用或没有合适题目时，才 fallback 调用 create_coding_task 自拟题\n"
    ...
    "- 只有需要更换题目时才再次检索题库并创建新题；同一道题的修改必须使用 request_coding_revision\n\n"
    "注意事项：\n"
    "- 保持专业但友好的语气\n"

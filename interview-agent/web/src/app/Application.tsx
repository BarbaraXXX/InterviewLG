...
import { ThemeToggle, type ThemeMode } from '../components/theme';
import {
  adminLogin,
  adminLogout,
  createSession,
...
function DeferredMarkdown({ content }: { content: string }) {
  return (
    <Suspense fallback={<span className="markdown-loading">正在加载内容...</span>}>
...
function LoginView({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authRequestGenerationRef.current !== requestGeneration) return;
      onLogin(result.username);
    } catch (err) {
      if (authRequestGenerationRef.current !== requestGeneration) return;
      setError(err instanceof Error ? err.message : '操作失败');
    } finally {
      if (authRequestGenerationRef.current === requestGeneration) {
        setLoading(false);
      }
    }
  };
  ...
  return (
    <main className="setup-view auth-page">
      <div className="auth-shell">
        <section className="auth-card" aria-label={isRegister ? '注册' : '登录'}>
          <div className="panel-heading">
            <p className="eyebrow">{isRegister ? '加入问砺' : '欢迎回来'}</p>
            <h1>{isRegister ? '创建账号' : '登录账号'}</h1>
            <p>{isRegister ? '当前处于邀请制内测，使用邀请码创建账号。' : '继续你的技术面试表达训练。'}</p>
          </div>
          ...
          <form className="login-form" onSubmit={handleSubmit} aria-busy={loading}>
            ...
            <p className="auth-submit-status" aria-live="polite">
              {loading ? '正在验证账号信息，请稍候。' : ''}
            </p>

            <button
              className="start-button"
              type="submit"
              disabled={loading || !username || !password || (isRegister && !inviteCode.trim())}
            >
              {loading ? '请稍候...' : isRegister ? '创建账号' : '进入问砺'}
            </button>
          </form>

          <button className="auth-back-link" type="button" onClick={handleHome}>
            返回产品首页
          </button>
        </section>
        <section className="auth-command" aria-label="产品介绍">
          <div className="auth-command-top">
            <button className="auth-home-button" type="button" onClick={handleHome}>
              <BrandMark size="md" accessibleLabel="返回问砺首页" />
            </button>
            <div className="auth-command-actions">
              <span className="system-pill">邀请制内测</span>
              <ThemeToggle theme={theme} onToggle={onToggleTheme} />
            </div>
          </div>

          <div className="auth-hero-copy">
            <p className="eyebrow">AI 技术面试训练</p>
            <h2>把掌握的知识，练成面试现场能说清楚的回答</h2>
            <p>
              问砺通过连续对话模拟技术面试中的思考与表达过程。它会沿着你的项目经历、技术选择和每一次回答继续追问，让练习逐渐形成能够讲清原理、说明取舍、应对深入追问的表达能力。
            </p>
          </div>

          <div className="auth-dialogue-preview" aria-label="连续追问示意">
            <div className="auth-dialogue-meta">
              <span>项目深挖</span>
              <strong>03 / 10</strong>
            </div>
            <div className="auth-dialogue-turn">
              <span>面试官</span>
              <p>你提到用缓存承接热点请求。缓存失效的瞬间，怎样避免请求同时落到数据库？</p>
            </div>
            <div className="auth-dialogue-cue">
              <span aria-hidden="true" />
              <p>从“知道方案”继续走到“能解释边界与取舍”。</p>
            </div>
          </div>

          <p className="auth-privacy-copy">
            简历与岗位信息均可选；练习内容会由部署方配置的模型服务处理，请勿填写敏感个人信息。
          </p>
        </section>
      </div>
    </main>
  ...
  return (
    <div className="setup-view">
      <div className="console-shell dashboard-shell">
        <ConsoleTopbar title="问砺工作台" username={username} theme={theme} onToggleTheme={onToggleTheme} onLogout={onLogout} />
                    ...
                    if (timelineItem.type === 'coding') {
                      ...
                      return (
                        <article className="history-coding-card" key={`coding-${task.id}`}>
                          ...
                          <h3>{task.title}</h3>
                          <DeferredMarkdown content={task.description} />
                          {task.submitted_code ? (
                    ...
                    return (
                      <article className="qa-card" key={`qa-${pair.question.seq}-${timelineItem.qaIndex}`}>
                        ...
                        <div className="qa-message user">
                          <strong>我的回答</strong>
                          <DeferredMarkdown content={pair.question.content} />
                        </div>
                        <div className="qa-message ai">
                          <strong>AI 面试官</strong>
                          <DeferredMarkdown content={pair.answer?.content || '这轮还没有 AI 回复。'} />
                        </div>
                      </article>
                    );
                  })}
...
function SetupView({ onStart, username, theme, onToggleTheme, onAuthExpired, onLogout, onBack, onProfile }: {
  onStart: (
    domain: string,
    difficulty: string,
  ...
  const handleOptionalLoadError = useCallback((err: unknown) => {
    if (err instanceof Error && err.message === 'UNAUTHORIZED') {
      onAuthExpired();
    }
  }, [onAuthExpired]);
  ...
  useEffect(() => {
    fetchDomains().then(setDomains).catch(handleOptionalLoadError);
  }, [handleOptionalLoadError]);

  useEffect(() => {
    fetchProfiles().then(setProfiles).catch(handleOptionalLoadError);
  }, [handleOptionalLoadError]);
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`message-row ${msg.role === 'user' ? 'user' : 'ai'}`}>
                ...
                <div className={`message-bubble ${msg.role}`}>
                  <DeferredMarkdown content={msg.content} />
...
function AdminApp({ theme, onToggleTheme }: ThemeControllerProps) {
  ...
  const handleLogout = useCallback(async () => {
    try {
      ...
      setAuthError('管理员退出失败，当前登录状态仍然保留，请检查网络后重试。');
    }
  }, [navigate]);
  ...
  if (!username || location.pathname === ROUTES.adminLogin) {
    return <AdminLoginView theme={theme} onToggleTheme={onToggleTheme} onLogin={handleLogin} />;
  }
  return <AdminDashboardView username={username} theme={theme} onToggleTheme={onToggleTheme} onLogout={handleLogout} />;
}

function UserApp({ theme, onToggleTheme }: ThemeControllerProps) {
  const location = useLocation();
  const navigate = useNavigate();
  ...
  const clearInterviewRuntimeState = useCallback(() => {
    setSessionId('');
    setDomain('');
    setDifficulty('');
    setChatMessages([]);
  ...
  const handleAuthExpired = useCallback((expectedGeneration?: number) => {
    if (
      (expectedGeneration !== undefined && expectedGeneration !== authGenerationRef.current)
    ...
    if (latestPathRef.current !== ROUTES.login) {
      const nextPath = isProtectedUserRoute(latestPathRef.current)
        ? `${latestPathRef.current}${window.location.search}${window.location.hash}`
        : null;
      const loginRoute = nextPath ? createLoginRoute(nextPath) : ROUTES.login;
      latestPathRef.current = ROUTES.login;
      navigate(loginRoute, { replace: true });
    }
  }, [clearInterviewRuntimeState, navigate]);
    ...
    void getMe()
      .then((me) => {
        ...
        if (me) {
          ...
          const requestedRoute = latestPath === ROUTES.login
            ? resolveLoginNextPath(new URLSearchParams(window.location.search).get('next'))
            : `${latestPath}${window.location.search}${window.location.hash}`;
          const requestedPathname = new URL(requestedRoute, window.location.origin).pathname;
          const requestedView = routeToUserView(requestedPathname);
          ...
          const destinationRoute = requestedView && requestedView !== 'login' ? requestedRoute : nextRoute;
          latestPathRef.current = new URL(destinationRoute, window.location.origin).pathname;
          if (`${window.location.pathname}${window.location.search}${window.location.hash}` !== destinationRoute) {
  ...
  }, [authRetryNonce, clearInterviewRuntimeState, navigate]);
  ...
  useEffect(() => {
    const heartbeatGeneration = authGenerationRef.current;
    ...
    const sendHeartbeat = () => {
      ...
      void sendPresenceHeartbeat(heartbeatView, heartbeatView === 'chat' ? sessionId : '')
        .catch((err) => {
          if (err instanceof Error && err.message === 'UNAUTHORIZED') {
            handleAuthExpired(heartbeatGeneration);
          }
        });
    };
  ...
  const handleLogin = (user: string) => {
    ...
    setHistoryNoticeDismissed(false);
    setHistoryManageModeDefault(false);
    const nextRoute = resolveLoginNextPath(new URLSearchParams(location.search).get('next'));
    const nextPathname = new URL(nextRoute, window.location.origin).pathname;
    const nextView = routeToUserView(nextPathname) ?? 'dashboard';
    latestPathRef.current = nextPathname;
    setView(nextView);
    navigate(nextRoute, { replace: true });
  };

  const handleLogout = useCallback(async (expectedGeneration?: number) => {
    if (expectedGeneration !== undefined && expectedGeneration !== authGenerationRef.current) return;
    ...
    try {
      ...
      setAuthRetryMessage('');
      setView('login');
      latestPathRef.current = ROUTES.root;
      navigate(ROUTES.root, { replace: true });
    } catch {
      setResourceLoadError('退出登录失败，当前登录状态仍然保留，请检查网络后重试。');
    } finally {
  ...
  const handleCurrentAuthExpired = useCallback(
    () => handleAuthExpired(currentAuthGeneration),
    [currentAuthGeneration, handleAuthExpired],
  );
  const handleCurrentLogout = useCallback(
    () => void handleLogout(currentAuthGeneration),
    [currentAuthGeneration, handleLogout],
  );
  ...
  }, [difficulty, domain, handleCurrentAuthExpired, interviewRouteSessionId, navigate, sessionId, username]);
  ...
  const handleCurrentResume = (detail: InterviewSessionDetail) => {
    if (currentAuthGeneration !== authGenerationRef.current) return;
    handleResume(detail);
  };
  ...
  const loginNextPath = resolveLoginNextPath(new URLSearchParams(location.search).get('next'));
  ...
  return (
    <div className={`app-shell ${showMobileNavigation ? 'has-mobile-navigation' : ''}`}>
      {username && view !== 'loading' && view !== 'chat' && routeView === 'login' && (
        <Navigate to={loginNextPath} replace />
      )}
      {activeView === 'loading' && (
        <LoadingView
      ...
      {activeView === 'dashboard' && (
        <DashboardView
          ...
          onAuthExpired={handleCurrentAuthExpired}
          onLogout={handleCurrentLogout}
      ...
      {activeView === 'history' && (
        <HistoryView
          ...
          onResumeInterview={handleCurrentResume}
          onAuthExpired={handleCurrentAuthExpired}
          onLogout={handleCurrentLogout}
        />
      )}
      ...
      <footer className={`site-footer ${showMobileNavigation ? 'with-mobile-navigation' : ''}`}>
        <span className="app-version">问砺 {APP_VERSION}</span>
...
function App({ theme, onToggleTheme }: ThemeControllerProps) {
  return (
    <Routes>
      {ADMIN_ROUTE_ENTRIES.map((route) => (
        <Route
          key={route.path}
          path={route.path}
          element={<AdminApp theme={theme} onToggleTheme={onToggleTheme} />}
        />
      ))}
      <Route path="*" element={<UserApp theme={theme} onToggleTheme={onToggleTheme} />} />
    </Routes>
  );
}
